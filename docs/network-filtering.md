...
Opt out per instance with `cogbox l7 mode passthrough`, per host with `cogbox l7 add allow <host> --passthrough`, or by setting `.network.l7.inject` to `false`. An explicit `COGBOX_L7_INJECT_CONF=<path>` overrides the generated conf (used for testing or a hand-rolled mapping).
